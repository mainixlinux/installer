...
def setup_autologin_oobe(mount_point):
    oobe_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "oobe.py")
    if os.path.exists(oobe_path):
        shutil.copy(oobe_path, f"{mount_point}/tmp/oobe.py")
        os.chmod(f"{mount_point}/tmp/oobe.py", 0o755)
    else:
        raise FileNotFoundError("oobe.py not found in installer directory!")
